...
Future<int?> showDownloadSelectionSheet({
  ...
  required DownloadViewModel viewModel,
}) {
...
class _DownloadSelectionSheetState extends State<_DownloadSelectionSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return FractionallySizedBox(
      ...
      child: Padding(
        ...
        child: Column(
          mainAxisSize: episodes.length > 6
              ? MainAxisSize.max
              : MainAxisSize.min,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return CheckboxListTile(
          ...
          subtitle: index == widget.currentEpisodeIndex
              ? const Text('当前播放')
              : null,
  ...
  Future<void> _enqueueSelected() async {
    ...
    try {
      final result = await widget.viewModel.enqueueEpisodes(
        detail: widget.detail,
        episodeIndexes: _selectedIndexes,
      );
      if (!mounted) return;
      if (result.isFailure) {
        throw result.failureOrNull!;
      }
      Navigator.of(context).pop(result.valueOrNull!.length);
    } catch (_) {
      if (!mounted) return;
      setState(() => _submitting = false);
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('创建下载任务失败，请稍后重试')));
    }
  }
