...
import 'package:selene/domain/models/aggregated_search_result.dart';
  ...
  void _updateAggregatedResults() {
    ...
    for (final result in widget.results) {
      ...
      if (newAggregatedResults.containsKey(key)) {
        // 已存在，添加到现有聚合结果中
        newAggregatedResults[key] = newAggregatedResults[key]!.addResult(
          result,
        );
      } else {
        // 新的聚合结果
        newAggregatedResults[key] = AggregatedSearchResult.fromSearchResult(
          result,
        );
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.search_off, size: 80, color: const Color(0xFFbdc3c7)),
