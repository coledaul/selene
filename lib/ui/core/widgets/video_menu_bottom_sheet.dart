...
import 'package:selene/ui/core/view_models/video_metadata_view_model.dart';
import 'package:selene/ui/core/widgets/video_metadata_scope.dart';
...
class VideoMenuBottomSheet extends StatefulWidget {
  ...
  final VideoMetadataViewModel viewModel;
  ...
  static void show(
    ...
    final viewModel = VideoMetadataScope.createViewModel(context);
...
class _VideoMenuBottomSheetState extends State<VideoMenuBottomSheet>
  ...
  DoubanMovieDetails? get _doubanDetails =>
      widget.viewModel.state.doubanDetails;
  bool get _isLoadingDoubanDetails => widget.viewModel.state.loadingDouban;
  BangumiDetails? get _bangumiDetails => widget.viewModel.state.bangumiDetails;
  bool get _isLoadingBangumiDetails => widget.viewModel.state.loadingBangumi;
  ...
  void initState() {
    super.initState();
    widget.viewModel.addListener(_handleMetadataChanged);
  ...
  void dispose() {
    widget.viewModel
      ..removeListener(_handleMetadataChanged)
      ..dispose();
  ...
  void _handleMetadataChanged() {
    ...
    if (_doubanDetails != null || _bangumiDetails != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _calculateContentBasedMaxHeight();
      });
    }
  }
  ...
  void _captureInitialHeight() {
    try {
      ...
      AppLogger.debug('捕获视频菜单初始高度失败', error: e);
    }
  }
  ...
  void _calculateContentBasedMaxHeight() {
    ...
    try {
      ...
      AppLogger.debug('计算视频菜单内容高度失败', error: e);
    }
  }
  ...
  void _animateToHeight(double targetHeight) {
    ...
    // 使用更高效的动画曲线
    _transitionAnimation = Tween<double>(begin: startHeight, end: targetHeight)
        .animate(
          CurvedAnimation(
  ...
  Future<void> _loadDoubanDetails(String doubanId) async {
    ...
    await widget.viewModel.loadDouban(doubanId);
    if (!mounted || _doubanDetails == null) return;
    setState(() {
      _showScrollIndicator =
          _hasInitialHeightCaptured &&
  ...
  Future<void> _loadBangumiDetails(String bangumiId) async {
    ...
    await widget.viewModel.loadBangumi(bangumiId);
    if (!mounted || _bangumiDetails == null) return;
    setState(() {
      _showScrollIndicator =
          _hasInitialHeightCaptured &&
          (_currentSheetHeight == null ||
                            ...
                            child: NotificationListener<ScrollNotification>(
                              onNotification: (notification) {
                                ...
                                if (notification is OverscrollNotification) {
                                  ...
                                  final velocityThreshold = _isIOS
                                      ? 400.0
                                      : 800.0;
  ...
  /// 构建下滑指示器
  Widget _buildScrollIndicator(ThemeViewModel themeService) {
  ...
  List<Widget> _buildBangumiInfoboxDetails(ThemeViewModel themeService) {
  ...
  /// 构建菜单选项
  Widget _buildMenuOptions(BuildContext context, ThemeViewModel themeService) {
    // 如果是豆瓣来源，只显示播放和豆瓣详情
        ...
        children: [
          ...
          _buildMenuItem(
            ...
            onTap: () async {
              ...
              final doubanId =
                  (widget.videoInfo.doubanId != null &&
  ...
  /// 构建分割线
  Widget _buildDivider(ThemeViewModel themeService) {
    return Container(
      height: 1,
  ...
  void _showSourcesDialog() {
    final themeService = Provider.of<ThemeViewModel>(context, listen: false);
