...
enum RecommendationRefreshStatus { idle, refreshing, failed }
...
class RecommendationSection extends StatefulWidget {
  ...
  final RecommendationRefreshStatus refreshStatus; // 保留现有内容时的刷新状态
  ...
  const RecommendationSection({
    ...
    this.hasError = false,
    this.refreshStatus = RecommendationRefreshStatus.idle,
    this.onRetry,
    this.cardCount = 2.75,
  });
...
class _RecommendationSectionState extends State<RecommendationSection> {
  ...
  void _scrollLeft() {
    ...
    // 根据可见卡片数动态计算滚动距离
    final double visibleCards = DeviceUtils.getHorizontalVisibleCards(
      context,
      widget.cardCount,
    );
  ...
  Widget build(BuildContext context) {
    ...
    // 如果没有数据且不在加载中，隐藏组件
    if (!widget.isLoading && !widget.hasError && currentItems.isEmpty) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.refreshStatus != RecommendationRefreshStatus.idle) ...[
  ...
  Widget _buildRefreshStatus() {
    return Padding(
      ...
      child: Row(
        children: [
          if (widget.refreshStatus ==
              RecommendationRefreshStatus.refreshing) ...[
            const SizedBox(
              width: 12,
              height: 12,
            ...
            Icon(Icons.error_outline, color: Colors.grey[500], size: 14),
            const SizedBox(width: 6),
            Text(
              '刷新失败',
              style: FontUtils.poppins(fontSize: 12, color: Colors.grey[600]),
            ),
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              '加载失败',
              style: FontUtils.poppins(fontSize: 14, color: Colors.grey[600]),
            ),
            if (widget.onRetry != null) ...[
              const SizedBox(height: 8),
              TextButton(
