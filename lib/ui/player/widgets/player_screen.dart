...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  // 播放信息
  SearchResult? get currentDetail => _viewModel.state.currentDetail;
  String get searchTitle => _viewModel.state.searchTitle;
  String get videoTitle => _viewModel.state.videoTitle;
  String get videoDesc => _viewModel.state.videoDescription;
  String get videoYear => _viewModel.state.videoYear;
  String get videoCover => _viewModel.state.videoCover;
  int get videoDoubanID => _viewModel.state.doubanId;
  String get currentSource => _viewModel.state.currentSource;
  String get currentID => _viewModel.state.currentId;
  bool get needPrefer => _viewModel.state.preferSource;
  int get totalEpisodes => _viewModel.state.totalEpisodes;
  int get currentEpisodeIndex => _viewModel.state.currentEpisodeIndex;
  DoubanMovieDetails? get doubanDetails => _viewModel.state.doubanDetails;
  List<SearchResult> get allSources => _viewModel.state.sources;
  Map<String, SourceSpeed> get allSourcesSpeed => _viewModel.state.sourceSpeeds;

  // VideoPlayerWidget 的控制器
  VideoPlayerWidgetController? _videoPlayerController;

  // 收藏状态
  bool get _isFavorite => _viewModel.state.favorite;
    ...
    if (mounted) {
      ...
      setState(() {
        if (state.loadingMessage.isNotEmpty) {
          _loadingMessage = state.loadingMessage;
          _loadingEmoji = state.loadingEmoji;
  ...
  Future<void> initVideoData() async {
    ...
    if (!_isActiveLoad(loadGeneration)) return;
    startPlay(
      _viewModel.state.resumeEpisodeIndex,
      _viewModel.state.resumePlayTime,
    );
  }
  ...
  void startPlay(int targetIndex, int playTime) {
    ...
    _viewModel.setEpisode(targetIndex);
    // 重置上次保存的位置，因为切换了集数
  ...
  void setInfosByDetail(SearchResult detail) {
    if (_viewModel.applyDetail(detail)) {
      _fetchDoubanDetails();
    }
  ...
  Future<void> _fetchDoubanDetails() async {
    if (videoDoubanID <= 0) {
      _viewModel.setDoubanDetails(null);
      return;
    }

    try {
      final requestDoubanId = videoDoubanID.toString();
      final response = await _viewModel.getDoubanDetails(videoDoubanID);
      if (!mounted || videoDoubanID.toString() != requestDoubanId) return;

      if (response case Success(:final value) when value != null) {
        _viewModel.setDoubanDetails(value);
      } else {
        AppLogger.debug('获取豆瓣详情失败', error: response.failureOrNull);
      }
    } catch (e) {
      AppLogger.debug('获取豆瓣详情异常', error: e);
    }
  }
  ...
  void _saveProgress({bool force = false, required String scene}) {
    try {
      ...
      _viewModel
          ...
          .then((result) {
            if (result.isFailure) {
              AppLogger.debug('保存播放进度失败 [$scene]', error: result.failureOrNull);
              return;
            }
            debugPrint(
  ...
  Future<void> _showDownloadSelector() async {
    ...
    final count = await showDownloadSelectionSheet(
      ...
      viewModel: _viewModel.downloads,
    );
    if (!mounted || count == null) return;
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('已加入 $count 个下载任务')));
  }
  ...
  void _checkFavoriteStatus() {
    if (currentSource.isNotEmpty && currentID.isNotEmpty) {
      final isFavorited = _viewModel.isFavorited(currentSource, currentID);
      if (mounted) {
        _viewModel.updateFavoriteState(isFavorited);
      }
    }
  }
  ...
  void _toggleFavorite() async {
    ...
    final result = await _viewModel.setFavorite(
      favorite: desired,
      record: _currentPlayRecord(playTime: 0, totalTime: 0),
    );
    if (!mounted) return;
    if (result case Success(:final value)) {
      _viewModel.updateFavoriteState(value);
    }
  }
  ...
  void _performScrollToCurrentSource() {
    ...
    final targetOffset =
        (currentSourceIndex * (cardWidth + itemMargin)) -
  ...
  void _switchSource(SearchResult newSource) async {
    ...
    if (oldSource.isNotEmpty &&
      ...
      try {
        await _viewModel.deletePlayRecord(oldSource, oldID);
        if (!mounted) return;
    ...
    _viewModel.setEpisode(currentEpisode);
  ...
  void _performScrollToCurrentEpisode() {
    ...
    // 注意：要减去第一个item的左边距（因为ListView有左padding）
    final targetOffset =
        (targetIndex * (buttonWidth + itemMargin)) -
        (visibleCenter - itemCenter - listViewPadding);
  ...
  void _onRecommendTap(DoubanRecommendItem recommend) {
    ...
    // 跳转到新的播放页，只传递title参数
    PlayerRoute(title: recommend.title).push<void>(context);
  }
  ...
  void _showEpisodesPanel() {
    ...
    if (_isTablet) {
      ...
      final panelHeight = _isPortraitTablet
          ...
          : screenHeight;
      final alignment = _isPortraitTablet
          ? Alignment.bottomCenter
          : Alignment.centerRight;
      final slideBegin = _isPortraitTablet
          ? const Offset(0, 1)
          : const Offset(1, 0);
  ...
  Future<void> _refreshSourcesSpeed([StateSetter? stateSetter]) async {
    ...
    try {
      // 清空之前的测速结果
      _viewModel.clearSourceSpeeds();

      // 立即更新UI显示，让用户看到测速信息被清空
      aSetState(() {});

      await _viewModel.testSources(
        allSources,
        onResult: (sourceId, speed) {
          _viewModel.setSourceSpeed(sourceId, speed);
          aSetState(() {});
        },
      );
    } catch (e) {
          ...
          child: Column(
            children: [
              ...
              if (Platform.isWindows)
                const WindowsTitleBar(customBackgroundColor: Color(0xFF000000)),
              // 主要内容
              Expanded(
                child: Stack(
                  children: [
    ...
    if (_isWebFullscreen) {
      ...
      return Positioned(
        ...
        child: Column(
          children: [
            // 顶部安全区域
            Container(height: topOffset, color: Colors.black),
            // 播放器
            Expanded(
              child: Container(
                key: _playerKey,
  ...
  Widget _buildPhoneLayout(ThemeData theme) {
    ...
    return Column(
      children: [
        Container(height: statusBarHeight + macOSPadding, color: Colors.black),
        // 播放器占位空间
        SizedBox(height: playerHeight),
        Expanded(child: _buildVideoDetailSection(theme)),
      ],
    );
  }
...
class _HoverButton extends StatefulWidget {
  ...
  const _HoverButton({required this.child, this.onTap, this.enabled = true});
