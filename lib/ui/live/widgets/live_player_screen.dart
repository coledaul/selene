...
import 'package:selene/ui/live/view_models/live_player_ui_state.dart';
...
class _LivePlayerScreenState extends State<LivePlayerScreen>
  ...
  LiveChannel get _currentChannel => _state.currentChannel;
  LiveSource get _currentSource => _state.currentSource;
  List<EpgProgram>? get _programs => _state.programs;
  bool get _isLoadingEpg => _state.loadingEpg;
  List<LiveChannel> get _allChannels => _state.channels;
  List<LiveSource> get _allSources => _state.sources;
  ...
  Future<void> _switchChannel(LiveChannel channel) async {
    ...
    await _viewModel.switchChannel(channel);
  }
  ...
  void _scrollToCurrentChannel() {
    ...
    // 找到当前频道的索引
    final currentIndex = filteredChannels.indexWhere(
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isDarkMode = theme.brightness == Brightness.dark;
    final themeService = context.watch<ThemeViewModel>();
  ...
  /// 构建手机模式布局
  Widget _buildPhoneLayout(ThemeData theme, ThemeViewModel themeService) {
    final statusBarHeight = MediaQuery.maybeOf(context)?.padding.top ?? 0;
    final macOSPadding = DeviceUtils.isMacOS() ? 32.0 : 0.0;
    final screenWidth = MediaQuery.of(context).size.width;
    final playerHeight = screenWidth / (16 / 9);
    ...
    return Column(
      children: [
        ...
        // 频道列表（占据剩余空间）
        Expanded(child: _buildChannelList(theme, themeService)),
      ],
    );
  }
  ...
  /// 构建频道信息
  Widget _buildChannelInfo(ThemeData theme, ThemeViewModel themeService) {
    return Container(
  ...
  Widget _buildDefaultLogo(ThemeViewModel themeService) {
    return SizedBox(
      height: 56,
      child: AspectRatio(
        aspectRatio: 2.0,
        child: Container(
          ...
          child: const Icon(Icons.tv, size: 24, color: Color(0xFF95a5b0)),
        ),
      ),
    );
  }
  ...
  Widget _buildDefaultLogoIcon() {
    return const Icon(Icons.tv, size: 24, color: Color(0xFF95a5b0));
  }

  /// 构建频道列表
  Widget _buildChannelList(ThemeData theme, ThemeViewModel themeService) {
    final filteredChannels = _getFilteredChannels();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // 只给当前频道添加 key，用于滚动定位
        final itemKey = isSelected
            ? _currentChannelKey
            : ValueKey('channel_${channel.id}');
  ...
  /// 构建播放源和分组选择器
  Widget _buildSourceSelector(ThemeData theme, ThemeViewModel themeService) {
    // 获取所有分组，保持原始顺序
    final allGroups = ['全部'];
        ...
        children: [
          ...
          if (showSourceFilter) ...[
            _buildFilterPill('直播源', sourceOptions, _currentSource.key, (
              ...
              await _viewModel.switchSource(source);
            }, themeService),
            const SizedBox(width: 8),
          ],
          // 分组筛选
          _buildFilterPill('分组', groupOptions, _selectedGroup, (value) {
            _viewModel.selectGroup(value);
          }, themeService),
          const Spacer(),
          // 滚动到当前频道按钮
          Padding(
  ...
  /// 构建滚动到当前频道按钮
  Widget _buildScrollToCurrentChannelButton(ThemeViewModel themeService) {
    return _HoverButton(
  ...
  /// 显示节目单下拉列表
  void _showProgramListDropdown(ThemeData theme, ThemeViewModel themeService) {
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              // 节目列表
              Expanded(child: _buildVerticalProgramList(themeService)),
            ],
          ),
        );
      },
  ...
  /// 构建垂直节目列表
  Widget _buildVerticalProgramList(ThemeViewModel themeService) {
    if (_isLoadingEpg) {
      return Center(
        child: Padding(
  ...
  /// 构建滚动到当前节目按钮
  Widget _buildScrollToCurrentButton(ThemeViewModel themeService) {
    return _HoverButton(
  ...
  /// 构建节目列表
  Widget _buildProgramList(ThemeViewModel themeService) {
    if (_isLoadingEpg) {
      return Center(
        child: Padding(
          padding: const EdgeInsets.symmetric(vertical: 24),
