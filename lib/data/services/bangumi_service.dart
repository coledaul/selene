...
class BangumiService {
  ...
  /// 获取当天的新番放送（根据当前星期几）
  static Future<ApiResponse<List<BangumiItem>>> getTodayCalendar() async {
    final weekday = DateTime.now().weekday; // 1..7
    return getCalendarByWeekday(weekday);
  }
  ...
  static Future<ApiResponse<BangumiDetails>> getBangumiDetails({
    ...
    // 尝试从缓存获取数据
    try {
      final cachedData = await _cache.get<BangumiDetails>(cacheKey, (raw) {
