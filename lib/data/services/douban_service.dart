...
class DoubanService {
  ...
  static Future<ApiResponse<List<DoubanMovie>>> getCategoryData({
    ...
    try {
      ...
      // 缓存读取失败，继续执行网络请求
      AppLogger.debug('读取豆瓣分类缓存失败', error: e);
    }
    // 获取用户存储的豆瓣数据源选项
      ...
      if (response.statusCode == 200) {
        try {
          ...
          try {
            ...
            AppLogger.debug('写入豆瓣分类缓存失败', error: cacheError);
          }

          return ApiResponse.success(
            doubanResponse.items,
  ...
  static Future<ApiResponse<List<DoubanMovie>>> fetchDoubanRecommends(
    ...
    try {
      ...
      // 缓存读取失败，继续执行网络请求
      AppLogger.debug('读取豆瓣推荐缓存失败', error: e);
    }
      ...
      if (response.statusCode == 200) {
        try {
          ...
          try {
            ...
            AppLogger.debug('写入豆瓣推荐缓存失败', error: cacheError);
          }
  ...
  static Future<ApiResponse<DoubanMovieDetails>> getDoubanDetails({
    ...
    // 尝试从缓存获取数据
    try {
      final cachedData = await _cacheService.get<DoubanMovieDetails>(cacheKey, (
        raw,
      ) {
        ...
        if (map['recommends'] != null) {
          ...
          recommends = recommendsData
              .map(
                (r) => DoubanRecommendItem.fromJson(r as Map<String, dynamic>),
              )
              .toList();
        }
      ...
      // 缓存读取失败，继续执行网络请求
      AppLogger.debug('读取豆瓣详情缓存失败', error: e);
    }
    ...
    switch (dataSourceKey) {
      case 'cdn_tencent':
        apiUrl =
            'https://m.douban.cmliussss.net/rexxar/api/v2/subject/$doubanId';
        break;
      case 'cdn_aliyun':
        apiUrl =
            'https://m.douban.cmliussss.com/rexxar/api/v2/subject/$doubanId';
        break;
      case 'direct':
      default:
      ...
      if (response.statusCode == 200) {
        try {
          ...
          try {
            ...
            AppLogger.debug('写入豆瓣详情缓存失败', error: cacheError);
          }
