...
/// M3U8 解析和播放源测速服务。
abstract interface class M3U8Analyzer {
  Future<Map<String, dynamic>> preferBestSource(List<SearchResult> sources);
...
class M3U8Service implements M3U8Analyzer {
  final Dio _dio = Dio();
  ...
  Future<Map<String, dynamic>> preferBestSource(
    ...
    // 计算基准值
    final maxSpeed = validSpeeds.isNotEmpty
        ? validSpeeds.reduce((a, b) => a > b ? a : b)
        : 1024.0; // 默认1MB/s作为基准
    final minPing = validPings.isNotEmpty
        ? validPings.reduce((a, b) => a < b ? a : b)
        : 50;
    final maxPing = validPings.isNotEmpty
        ? validPings.reduce((a, b) => a > b ? a : b)
        : 1000;
    ...
    final bestSource = sourceScores.isNotEmpty
        ? sourceScores.first.key
        : allSources.first;
