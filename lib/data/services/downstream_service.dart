...
class SearchPageResult {
  ...
  SearchPageResult({required this.results, required this.pageCount});
}
...
class DownstreamService {
  ...
  static Future<List<SearchResult>> searchFromApi(
    ...
    final apiUrl = '$apiBaseUrl?ac=videolist&wd=${Uri.encodeComponent(query)}';
  ...
  static Future<SearchPageResult> searchPage({
    ...
    if (cached != null) {
      ...
      throw StateError('搜索源缓存了失败状态: ${cached.status.name}');
    }
    ...
    try {
      ...
      if (response.statusCode == 403) {
        ...
        throw StateError('搜索源拒绝访问（HTTP 403）');
      }

      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw StateError('搜索请求失败（HTTP ${response.statusCode}）');
      }
      ...
      if (data is! Map<String, dynamic> || data['list'] is! List) {
        throw const FormatException('搜索响应格式无效');
      }

      final list = data['list'] as List;
      if (list.isEmpty) {
        return SearchPageResult(results: <SearchResult>[], pageCount: 1);
      }
      ...
      final allResults = list.map((item) {
        ...
        if (item['vod_play_url'] != null) {
          final vodPlayUrlArray = (item['vod_play_url'] as String).split(
            '\$\$\$',
          );
        ...
        if (item['vod_year'] != null && item['vod_year'] != '') {
          final yearMatch = RegExp(
            r'\d{4}',
          ).firstMatch(item['vod_year'] as String);
          if (yearMatch != null) {
        ...
        return {
          ...
          'title': (item['vod_name'] as String).trim().replaceAll(
            RegExp(r'\s+'),
            ' ',
          ),
          'poster': item['vod_pic'],
